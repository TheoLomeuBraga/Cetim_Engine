...
function summon_bullet(type,group,start,target_direction,speed,color)
    if global_data.core_object_ptr ~= nil then
        if core_obj == nil then
            core_obj = game_object(global_data.core_object_ptr)
        end
        ...
        core_obj.components.lua_scripts.scripts["game_scripts/bullet_master"].functions.summon_bullet({type,group,start,target_direction,speed,color})
    end
end
