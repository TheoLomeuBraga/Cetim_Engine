...
function START()
    ...
    walker_cube = create_mesh(this_object_ptr,false,path[1],{x=0, y=0, z=0},{x=1, y=1, z=1},2,{mat},{mesh_location:new("resources/3D Models/visual_debug_shapes.gltf","Cube")})

    for key, value in pairs(path) do
        local point = deepcopy(value)
        point.y = point.y + 2
        mat.color = {r=0,g=0,b=1,a=1}
        create_mesh(this_object_ptr,false,point,{x=0, y=0, z=0},{x=0.25, y=2, z=0.25},2,{mat},{mesh_location:new("resources/3D Models/visual_debug_shapes.gltf","Cube")})
    end
...
function UPDATE()
    ...
    if last_progression ~= nil then
        ...
        if walk_ret == nil then
            ...
            create_mesh(this_object_ptr,false,walk_ret.position,walk_ret.rotation,{x=0.2, y=0.2, z=1},2,{mat},{mesh_location:new("resources/3D Models/visual_debug_shapes.gltf","Cube")})
